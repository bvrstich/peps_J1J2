...
namespace propagate {
    ...
    */
   template<size_t M>
      void update_vertical(int row,int col,PEPS<double> &peps,const DArray<M> &L,const DArray<M> &R,int n_iter){
         ...
         construct_intermediate_vertical(row,col,peps,L,R,LI,RI);

         // --- (c) --- sweeping update
         sweep_vertical(row,col,peps,lop,rop,L,R,LI,RI,n_iter);
    ...
    */
   template<size_t M>
      void update_horizontal(int row,int col,PEPS<double> &peps,const DArray<M> &L,const DArray<M> &R,int n_iter){
         ...
         construct_intermediate_horizontal(row,col,peps,L,R,LI,RI);

         // --- (c) --- start sweeping
         sweep_horizontal(row,col,peps,lop,rop,L,R,LI,RI,n_iter);
   ...
   template<size_t M>
      void sweep_vertical(int row,int col,PEPS<double> &peps,const DArray<6> &lop,const DArray<6> &rop,
         ...
         while(iter < n_sweeps){
            ...
            construct_lin_sys_vertical(row,col,peps,lop,rop,N_eff,rhs,L,R,LI,RI,true);
            ...
            construct_lin_sys_vertical(row,col,peps,lop,rop,N_eff,rhs,L,R,LI,RI,false);
   ...
   template<size_t M>
      void sweep_horizontal(int row,int col,PEPS<double> &peps,const DArray<6> &lop,const DArray<6> &rop,
         ...
         while(iter < n_sweeps){
            ...
            construct_lin_sys_horizontal(row,col,peps,lop,rop,N_eff,rhs,L,R,LI,RI,true);
            ...
            construct_lin_sys_horizontal(row,col,peps,lop,rop,N_eff,rhs,L,R,LI,RI,false);
   ...
   /**
    * construct the single-site effective environment and right hand side needed for the linear system of the horinzontal gate,
    * for left or right site on middle rows (with R,L order 6)
    * @param row , the row index of the bottom site
    * @param col column index of the vertical column
    * @param peps, full PEPS object before update
    * @param N_eff output object, contains N_eff on output
    ...
    */
   template<>
      double cost_function_vertical(int row,int col,PEPS<double> &peps,const DArray<6> &lop,const DArray<6> &rop,
    ...
    * @param lop bottom peps acted onto with left trotter operator
    * @param rop upper peps acted onto with right trotter operator
    * @param peps_left bottom tensor, changed on output!
    * @param peps_right upper tensor, changed on output!
    */ 
   void initialize_horizontal(const DArray<6> &lop,const DArray<6> &rop,DArray<5> &peps_left,DArray<5> &peps_right){
      ...
      //svd the fucker
      DArray<1> S;
      Gesvd ('S','S', tmp8, S,peps_left,peps_right,D);
    ...
    * @param peps_up upper tensor, changed on output!
    */ 
   void equilibrate_horizontal(DArray<5> &peps_left,DArray<5> &peps_right){

      DArray<8> tmp8;
      Contract(1.0,peps_left,shape(4),peps_right,shape(0),0.0,tmp8);
    ...
    */
   template<>
      void construct_intermediate_vertical(int row,int col,const PEPS<double> &peps,const DArray<6> &LO,const DArray<6> &RO,DArray<8> &LI8,DArray<8> &RI8){
    ...
    */
   template<>
      void construct_intermediate_horizontal(int row,int col,const PEPS<double> &peps,const DArray<5> &L,const DArray<5> &R,DArray<7> &LI7,DArray<7> &RI7){
